...
class Signal:
    def __init__(self, map_name, sumo, id, yellow_length, phases):
        ...
        # Group of lanes constituting a direction of traffic读取路口信息并创建宏参数
    ...
    def get_emission(self):
        ...
        for lane in self.lanes:
            wait_time.append(self.sumo.lane.getLastStepHaltingNumber(lane))
            co2.append(self.sumo.lane.getCO2Emission(lane))
            hc.append(self.sumo.lane.getHCEmission(lane))
            nox.append(self.sumo.lane.getNOxEmission(lane))
        wait_car = sum(wait_time)
        CO2 = sum(co2)
        HC = sum(hc)
        NOX = sum(nox)

        return wait_car, CO2, HC, NOX


    def observe(self, step_length, distance):
        # print("ALL wait", self.waiting_times)
    ...
    def get_vehicles(self, lane, max_distance=200):
        detectable = []
