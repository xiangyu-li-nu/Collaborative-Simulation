...
class MultiSignal(gym.Env):
    def __init__(self, run_name, map_name, net, state_fn, reward_fn, eva, route=None, gui=False, end_time=3600,
        ...
        self.wait_time_all_epsiode = dict()
        self.co2_all_epsiode = dict()
        self.hc_all_epsiode = dict()
        self.nox_all_epsiode = dict()
        # Run some steps in the simulation with default light configurations to detect phases
        if self.route is not None:
            sumo_cmd = [sumolib.checkBinary('sumo'), '-n', net, '-r', self.route + '.rou.xml', '--no-warnings', 'True']
        else:
    ...
    def cc_pjzb(self):
        for tl in self.signal_ids:
            wait_car, co2, hc, nox = self.signals[tl].get_emission()
            self.wait_time_all_epsiode[tl].append(wait_car)
            self.co2_all_epsiode[tl].append(co2)
            self.hc_all_epsiode[tl].append(hc)
            self.nox_all_epsiode[tl].append(nox)
    ...
    def reset(self):
        if self.run != 0:
            if not self.libsumo: traci.switch(self.connection_name)
            traci.close()
            print('end', self.connection_name, self.run)
            # self.save_metrics()
        self.metrics = []
        ...
        self.eva(self.signals, reset=True)

        for ts in self.signal_ids:
            self.wait_time_all_epsiode[ts] = []
            self.co2_all_epsiode[ts] = []
            self.hc_all_epsiode[ts] = []
            self.nox_all_epsiode[ts] = []
    ...
    def step(self, act):
        ...
        delay, queue = self.eva(self.signals)

        self.calc_metrics(rewards)
        done = self.sumo.simulation.getTime() >= self.end_time
        # signal = self.signals['a']

        return observations, rewards, done, {'eps': self.run}, delay, queue
