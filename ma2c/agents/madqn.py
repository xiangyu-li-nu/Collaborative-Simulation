...
class DQNAgent(Agent):
    def __init__(self, config, act_space, model, num_agents=3):
        super().__init__()
    ...
    def load(self, m):
        self.model.load_state_dict(m['model_state_dict'])
        self.optimizer.load_state_dict(m['optimizer_state_dict'])
    ...
    def batch_act(self, batch_obs: Sequence[Any], valid_acts=None, reverse_valid=None) -> Sequence[Any]:
        ...
        if self.training:
            ...
            for i in range(len(batch_obs)):
                ...
                act, greedy = self.explorer.select_action(self.t, lambda: greed, action_value=av,
